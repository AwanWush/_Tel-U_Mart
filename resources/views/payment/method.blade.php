            ...
            document.getElementById('payBtn').addEventListener('click', async () => {
                ...
                try {
                    ...
                    window.snap.pay(data.snap_token, {
                        ...
                        onSuccess: (result) => {
                            window.location.href =
                                `/order/success?method=online&status=paid&amount=${amount}&order_id=${result.order_id}&payment_method=Midtrans%20Online&type=${type}&address=${encodeURIComponent(address)}`;
                        },
                        onPending: (result) => {
                            window.location.href =
                                `/order/success?method=online&status=pending&amount=${amount}&order_id=${result.order_id}&payment_method=Midtrans%20Online&type=${type}&address=${encodeURIComponent(address)}`;
                        },
                        onError: () => {
