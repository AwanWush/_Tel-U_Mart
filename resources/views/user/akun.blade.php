<x-app-layout>
    {{-- Header Section --}}
    <div class="bg-white border-b border-gray-200 mt-[120px]">
        ...
        <div class="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div class="w-full md:w-64 flex-shrink-0 sticky top-24">
                <div class="bg-white shadow sm:rounded-2xl overflow-hidden border border-gray-100">
                    <div class="p-6 flex flex-col items-center bg-white">
                        ...
                        <h3 class="text-base font-black text-slate-800 tracking-tight text-center">{{ $user->name }}</h3>
            ...
            <div class="w-full md:w-3/4">
                <div class="bg-white shadow sm:rounded-lg p-6">
                    ...
                    {{-- FLASH MESSAGE --}}
                    @if(session('success'))
                        <div class="mb-5 p-4 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm font-bold flex items-center gap-2">
                        ...
                        <div class="bg-white shadow sm:rounded-2xl border border-gray-100 p-6">
                                        ...
                                        <tbody>
                                            @forelse($riwayat as $item)
                                                <tr class="border-b hover:bg-gray-50">
                                                    <td class="px-4 py-2 border font-mono">{{ $item->id_transaksi }}</td>
                                                    <td class="px-4 py-2 border">{{ $item->created_at->format('d/m/Y H:i') }}</td>
                                                    <td class="px-4 py-2 border font-semibold">Rp{{ number_format($item->total_harga, 0, ',', '.') }}</td>
                                                    <td class="px-4 py-2 border">
                                                        <span class="px-2 py-1 rounded text-xs font-bold {{ $item->status == 'Lunas' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                                            {{ $item->status }}
                                                        </span>
                                                    </td>
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="5" class="text-center py-4">Belum ada riwayat pembelian.</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                            ...
                            <div id="detailModal"
                                ...
                                <div class="flex min-h-full items-start justify-center pt-20 pb-10">
                                    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col relative animate-in fade-in zoom-in duration-200 max-h-[80vh] overflow-y-auto">
                                        <div class="p-5 border-b flex justify-between items-center bg-gray-50/80 sticky top-0 z-10 backdrop-blur-md">
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div id="profil-view" class="space-y-6">
                                            ...
                                            <div class="flex items-center gap-4">
                                                <img src="{{ $imagePath }}" alt="Foto Profil"
                                                    class="w-16 h-16 object-cover rounded-full border-4 border-gray-100 shadow">
                                                <div>
                                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Foto Profil</p>
                                                    <p class="text-xs text-slate-500 mt-0.5">{{ $user->gambar ? 'Foto custom' : 'Foto default' }}</p>
                                                </div>
                                            </div>
                                        ...
                                        <div id="profil-form" class="hidden">
                                            <form action="{{ route('user.akun.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                                                @csrf
                                                @method('PATCH')
                                                {{-- Hidden field supaya redirect balik ke tab profil --}}
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                    ...
                                                    <div class="space-y-1.5">
                                                        ...
                                                        <select name="penghuni_asrama" id="penghuni_asrama"
                                                            class="w-full border-gray-200 rounded-xl focus:ring-[#dc2626] focus:border-[#dc2626] text-sm font-bold">
                                                            <option value="tidak" {{ old('penghuni_asrama', $user->penghuni_asrama) === 'tidak' ? 'selected' : '' }}>Tidak</option>
                                                            <option value="ya"    {{ old('penghuni_asrama', $user->penghuni_asrama) === 'ya'    ? 'selected' : '' }}>Ya</option>
                                                        </select>
                                                    </div>
                                                </div>

                                                {{-- ALAMAT GEDUNG --}}
                                                <div id="alamatGedungField" style="display: {{ old('penghuni_asrama', $user->penghuni_asrama) === 'ya' ? 'block' : 'none' }}"
                                                    class="space-y-4 p-5 bg-gray-50 rounded-2xl border border-gray-200">
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        <div class="space-y-1.5">
                                                            <label class="text-[10px] font-black text-[#b91c1c] uppercase tracking-widest ml-1">Alamat Gedung</label>
                                                            <select name="lokasi_id" class="w-full border-gray-200 rounded-xl text-sm font-bold focus:ring-[#dc2626]">
                                                                <option value="">-- Pilih Gedung --</option>
                                                                @foreach ($gedungs as $gedung)
                                                                    <option value="{{ $gedung->id }}" {{ old('lokasi_id', $user->lokasi_id) == $gedung->id ? 'selected' : '' }}>
                                                                        {{ $gedung->nama_lokasi }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                        ...
                                                        <div class="space-y-1.5">
                                                            ...
                                                            <select name="nomor_kamar" class="w-full border-gray-200 rounded-xl text-sm font-bold focus:ring-[#dc2626]">
                                                                <option value="">-- Pilih Nomor Kamar --</option>
                                                                @php
                                                                    $listKamar = \App\Models\MasterKamar::orderBy('nomor_kamar', 'asc')->get();
                                                                @endphp
                                                                @foreach ($listKamar as $kamar)
                                                                    <option value="{{ $kamar->nomor_kamar }}" {{ old('nomor_kamar', $user->nomor_kamar) == $kamar->nomor_kamar ? 'selected' : '' }}>
                                                    ...
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                        <div class="space-y-1.5">
                                                            ...
                                                            <input type="password" name="password" placeholder="Kosongkan jika tidak ingin diganti"
                                                                class="w-full border-gray-200 rounded-xl text-sm focus:ring-[#dc2626]">
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <div class="flex items-center gap-4 mb-2">
                                                        <img id="previewGambar" src="{{ $imagePath }}" alt="Preview"
                                                            class="w-14 h-14 object-cover rounded-full border-2 border-gray-200 shadow">
                                                        <p class="text-xs text-slate-400">Preview foto profil</p>
                                                    </div>

                                                    <input type="file" name="gambar" id="inputGambar" accept="image/jpg,image/jpeg,image/png,image/webp"
                                                        class="block w-full text-xs text-slate-500
                                                               file:mr-4 file:py-2 file:px-4
                                                               file:rounded-full file:border-0
                                                               file:text-xs file:font-bold
                                                               file:bg-[#fee2e2] file:text-[#dc2626]
                                                               hover:file:bg-[#fecaca] cursor-pointer">
                                                    <p class="text-[10px] text-slate-400 mt-1">Format: JPG, JPEG, PNG, WEBP. Maks 2MB.</p>
                            ...
                            <div id="tab-pembayaran" class="tab-content hidden">
                                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                    <div class="p-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                                        <div>
                                            <h3 class="text-lg font-black text-slate-800 uppercase tracking-tight">Dompet & Pembayaran</h3>
                                            <p class="text-xs text-slate-500">Kelola rincian pembayaran untuk transaksi Anda</p>
                                        </div>
                                    ...
                                    <div class="p-6">
                                        @if ($pembayaran->isEmpty())
                                            <div class="py-12 text-center">
                                                ...
                                                <p class="text-gray-500 font-bold text-sm">Belum ada metode pembayaran yang disimpan.</p>
                                            </div>
                                        @else
                                            <div class="grid grid-cols-1 gap-4">
                                                @foreach ($pembayaran as $pay)
                                                    <div class="group flex items-center justify-between p-4 rounded-2xl border border-gray-100 hover:border-[#DB4B3A]/30 hover:bg-gray-50 transition-all duration-300">
                                                        <div class="flex items-center gap-4">
                                                            <div class="w-12 h-12 rounded-xl bg-[#5B000B]/5 flex items-center justify-center text-[#930014]">
                                                            ...
                                                            <div>
                                                                <div class="flex items-center gap-2">
                                                                    <span class="font-black text-slate-800 uppercase text-sm tracking-tight">{{ $pay->kategori }}</span>
                                                                    @if ($pay->bank)
                                                                        <span class="px-2 py-0.5 bg-blue-100 text-blue-700 text-[9px] font-black rounded uppercase">{{ $pay->bank }}</span>
                                                                    @endif
                                                                </div>
                                                                <div class="mt-1 space-y-0.5">
                                                                    @if ($pay->telepon)
                                                                        <p class="text-xs text-slate-500 font-mono">ID: {{ $pay->telepon }}</p>
                                                                    @endif
                                                                    @if ($pay->norek)
                                                                        <p class="text-xs text-slate-500 font-mono tracking-wider">No: {{ $pay->norek }}</p>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                        </div>
    ...
    <script>
        ...
        function toggleGedung() {
            ...
            alamatField.style.display = asramaSelect.value === 'ya' ? 'block' : 'none';
            if (asramaSelect.value !== 'ya') {
                alamatField.querySelectorAll('select').forEach(s => s.value = '');
            }
        }
        ...
        if (editBtn)   editBtn.addEventListener('click',  () => { profilView.classList.add('hidden');    profilForm.classList.remove('hidden'); });
        if (cancelBtn) cancelBtn.addEventListener('click', () => { profilForm.classList.add('hidden');   profilView.classList.remove('hidden'); });

        // Jika ada error validasi, langsung buka form edit
        @if ($errors->any())
            if (profilView && profilForm) {
        ...
        if (inputGambar && previewGambar) {
            inputGambar.addEventListener('change', function () {
                ...
                if (file) {
                    ...
                    reader.onload = e => previewGambar.src = e.target.result;
        ...
        if (addPaymentBtn)    addPaymentBtn.addEventListener('click',    () => paymentModal.classList.remove('hidden'));
        if (cancelPaymentBtn) cancelPaymentBtn.addEventListener('click', () => paymentModal.classList.add('hidden'));
        ...
        function updateKategoriFields() {
            ...
            if (v === 'E-Wallet')        { ketField.classList.remove('hidden'); teleponField.classList.remove('hidden'); }
            if (v === 'Virtual Account') { bankField.classList.remove('hidden'); }
        }
        ...
        function showDetailOrder(data) {
            ...
            const lokasiTujuan  = isDelivery
                ? `<div class="flex items-center gap-2 font-bold text-slate-700"><i class="fas fa-map-marker-alt text-red-500"></i> ${data.alamat_tujuan}</div>`
                : `Ambil di Toko`;
            ...
            content.innerHTML = `
                <div class="space-y-8">
                    ...
                    <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                        ...
                        <div id="productList" class="px-4 bg-white border-t divide-y divide-gray-50 max-h-0 overflow-hidden transition-all duration-300">
                            ${itemsHtml}
                        </div>
                    </div>
