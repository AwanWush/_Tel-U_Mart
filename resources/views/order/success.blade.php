@php
    ...
    $status_config = [
        'paid'    => ['bg' => 'bg-emerald-500', 'text' => 'text-emerald-500', 'label' => 'PAID / LUNAS', 'icon' => 'fa-check-circle'],
        'pending' => ['bg' => 'bg-amber-500', 'text' => 'text-amber-500', 'label' => 'WAITING PAYMENT', 'icon' => 'fa-clock'],
        'unpaid'  => ['bg' => 'bg-indigo-600', 'text' => 'text-indigo-600', 'label' => 'PESANAN DITERIMA', 'icon' => 'fa-receipt']
    ];
...
<body class="bg-[#F3F4F6] min-h-screen">
    ...
    <main class="p-6 lg:p-10 max-w-[1200px] mx-auto">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
            ...
            <div class="no-print flex items-center gap-3">
                ...
                <a href="/" class="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">
            ...
            <div class="col-span-12 lg:col-span-8 space-y-8">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                        ...
                        <span class="px-3 py-1 bg-white border rounded-full text-[10px] font-black text-gray-400">{{ count($order_data) }} ITEM</span>
                    </div>
    ...
    {{ $item['store'] ?? 'T-Mart Central' }}
</p>
                    </div>
                </div>
            </td>
            <td class="px-8 py-5 text-center font-bold text-slate-900">{{ $item['qty'] }}</td>
    ...
    @endforeach
                ...
                <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                    ...
                    <div class="flex gap-4">
                        <div class="w-12 h-12 rounded-2xl {{ $is_delivery ? 'bg-red-50 text-red-500' : 'bg-indigo-50 text-indigo-600' }} flex items-center justify-center shrink-0">
                            <i class="fas {{ $is_delivery ? 'fa-map-marker-alt' : 'fa-store-alt' }} text-xl"></i>
                        </div>
                        <div>
                            <p class="font-bold text-slate-800">{{ $is_delivery ? 'Tujuan Pengiriman' : 'Lokasi Pengambilan Toko' }}</p>
                            <p class="text-sm text-gray-500 mt-1 leading-relaxed">
                                @if($is_delivery)
                                    {{ $delivery_address ?? 'Alamat tidak terdeteksi' }}
                                @else
                                    <span class="font-semibold text-slate-700">T-Mart Point Bandung.</span><br>
                                    Silahkan tunjukkan nomor Invoice di atas kepada petugas kasir untuk serah terima pesanan.
                                @endif
                            </p>
                        </div>
                    </div>
            ...
            <div class="col-span-12 lg:col-span-4 space-y-8">
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8 {{ $curr['bg'] }} text-white text-center">
                        <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-2">Status Pembayaran</p>
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <a href="/dashboard" class="no-print w-full py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest text-center block hover:bg-black hover:shadow-lg transition-all">
                            Kembali ke Beranda
                        </a>
                    </div>
                </div>
