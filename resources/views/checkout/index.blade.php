...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6">
        ...
        <nav class="flex text-gray-500 text-sm mb-6" aria-label="Breadcrumb">
            ...
            <ol class="inline-flex items-center space-x-1">
                ...
                <li>
                    <div class="flex items-center"><span class="mx-2">/</span><span
                            class="text-gray-800 font-semibold">Checkout</span></div>
                </li>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card-modern">
                    ...
                    <div class="flex p-1 bg-gray-100 rounded-2xl">
                        ...
                        <button id="btnDelivery"
                            class="flex-1 py-3 rounded-xl font-bold btn-active transition-all">Delivery</button>

                        <button id="btnTakeaway"
                            class="flex-1 py-3 rounded-xl font-bold text-gray-500 hover:text-gray-700 transition-all">Takeaway</button>
                ...
                <div id="addressContainer" class="card-modern">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button id="changeAddressBtn" class="text-red-600 text-sm font-bold hover:text-red-800">Ganti
                            Lokasi</button>
                    ...
                    <div class="p-4 bg-red-50 rounded-2xl border border-red-100">
                        ...
                        <p class="text-red-600 text-xs mt-1 italic">*Pastikan nomor kamar sudah benar untuk memudahkan
                            kurir</p>
                    ...
                    <div id="addressForm" class="hidden mt-6 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-1">
                                    ...
                                    <select id="gedungSelect" onchange="updateMapLocation()"
                                ...
                                <div class="space-y-1">
                                    ...
                                    {{-- PERBAIKAN: Ambil nomor kamar asli dari database --}}
                                    <input id="kamarInput" value="{{ Auth::user()->nomor_kamar ?? '' }}"
                        ...
                        <button onclick="saveNewAddress()"
                            class="w-full bg-red-600 text-white py-3 rounded-xl font-bold shadow-md hover:bg-red-700 transition active:scale-[0.98]">
                                    ...
                                    @foreach ($group['items'] as $item)
                                        <div class="flex justify-between items-center text-sm">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-gray-500">{{ $item['qty'] }} barang x
                                                    {{ currency($item['price']) }}</p>

                                            </div>

                                            <span
                                                class="font-bold text-gray-900">{{ currency($item['price'] * $item['qty']) }}</span>
                    ...
                    <div class="border-t border-dashed border-gray-200 pt-4 mb-6">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span id="finalPaymentAmount"
                                class="font-extrabold text-2xl text-red-600">{{ currency($total_payment) }}</span>
                    ...
                    <div class="bg-gray-50 p-3 rounded-xl mb-6">
                        ...
                        <label class="flex gap-3 text-xs items-start cursor-pointer group">
                            ...
                            <span class="text-gray-600 leading-tight group-hover:text-gray-800">
                                ...
                                Saya setuju dengan <a class="text-red-600 font-bold hover:underline">Syarat &
                                    Ketentuan</a> yang berlaku di Tel-U Mart.
    ...
    <script>
        ...
        function initMap() {
            ...
            // Ambil nama gedung dari database user (default Gedung 5)
            const userGedung = "{{ Auth::user()->alamat_gedung ?? 'Gedung 5' }}";

            // Gunakan koordinat gedung user dari daftar asramaCoords
            const initialCoords = asramaCoords[userGedung] || asramaCoords["Gedung 5"];

            map = L.map('map').setView(initialCoords, 18);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

            // Marker juga harus di lokasi gedung user
...
payBtn.onclick = () => {
    ...
    // Ambil alamat dari tampilan kartu
    const currentAddress = document.getElementById('displayGedung').innerText; 
    
    // Ambil total harga
    const totalAmount = finalPriceDisplay.innerText.replace(/[^0-9]/g, '');

    // --- TAMBAHAN UNTUK CHECKOUT LANGSUNG ---
    const urlParams = new URLSearchParams(window.location.search);
    const productId = urlParams.get('product_id') || ''; 
    const qty = urlParams.get('qty') || '';
    // ----------------------------------------

    // Pastikan parameter product_id dan qty ikut dikirim ke URL selanjutnya
    window.location.href = `/payment/method?type=${type}&amount=${totalAmount}&address=${encodeURIComponent(currentAddress)}&product_id=${productId}&qty=${qty}`;
};
    </script>
