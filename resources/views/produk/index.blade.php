<x-app-layout>
    {{-- 1. SCRIPT INSTAN: Membaca memori browser secepat mungkin --}}
    <script>
        if (localStorage.getItem('showAllCategories') === 'true') {
            document.documentElement.classList.add('kategori-is-open');
        }
    </script>

    {{-- 2. CSS PRIORITAS: Memaksa elemen tampil instan agar footer tidak naik --}}
    <style>
        /* Mencegah footer melompat dengan menyembunyikan konten x-cloak */
        [x-cloak] { display: none !important; }

        /* Jika status di memori browser true, paksa kategori extra tampil sejak detik pertama */
        .kategori-is-open .kategori-baris-extra { 
    ...
    <div class="pt-2 pb-8 bg-gray-50 min-h-screen font-sans" 
         x-data="{ 
            openFilter: false, 
            showAll: localStorage.getItem('showAllCategories') === 'true' 
         }" 
         x-init="
         ...
         x-effect="localStorage.setItem('showAllCategories', showAll)"
         x-cloak>
                ...
                <div class="relative inline-block text-left">
                    <button @click="openFilter = !openFilter" type="button"
                        ...
                        <svg class="ml-2 h-4 w-4 transition-transform duration-300"
                            :class="openFilter ? 'rotate-180' : ''" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 pt-4 pb-8">
                        ...
                        @forelse ($produk as $p)
                            <div x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false"
                                ...
                                <a href="{{ route('produk.show', $p) }}" class="block p-3 flex-grow text-left">
                                    ...
                                    <h3 class="mt-3 text-[13px] font-semibold text-gray-900 line-clamp-2 min-h-[34px] tracking-tight leading-tight">{{ $p->nama_produk }}</h3>
                                    ...
                                    <div class="mt-1 text-[10px] text-gray-500 leading-snug">
                                        ...
                                        <div class="flex flex-wrap gap-1">
                                            @foreach ($p->highlightedMarts() as $mart)
                                                <span class="{{ $mart['is_active'] ? 'text-[#930014] font-bold' : 'text-gray-400' }}">
                                                    {{ $mart['nama'] }}@if(!$loop->last),@endif
                                                </span>
                                            @endforeach
                                        </div>
                                    </div>
                                ...
                                <div x-show="hover" x-cloak x-transition.opacity class="absolute top-3 right-3 flex flex-col gap-2 z-10">
                                    <form action="{{ route('wishlist.store') }}" method="POST" @submit="localStorage.setItem('scrollPosition', window.scrollY)">
                                        @csrf
                                        <input type="hidden" name="produk_id" value="{{ $p->id }}">
                                        <button type="submit" class="w-9 h-9 rounded-full bg-[#E7BD8A]/80 hover:bg-[#E68757] border border-[#930014]/30 flex items-center justify-center text-[#930014] hover:text-white transition-all transform active:scale-75 shadow-sm">
                                            @include('icons.heart')
                                        </button>
                                    </form>

                                    <form action="{{ route('cart.store') }}" method="POST" @submit="localStorage.setItem('scrollPosition', window.scrollY)">
                                        @csrf
                                        <input type="hidden" name="produk_id" value="{{ $p->id }}">
                                        <input type="hidden" name="qty" value="1">
                                        <button type="submit" class="w-9 h-9 rounded-full bg-[#DB4B3A] hover:bg-[#930014] flex items-center justify-center text-white transition-all transform active:scale-75 shadow-sm">
                                            @include('icons.cart')
                                        </button>
                                    </form>
                                </div>
                ...
                @else
                    ...
                    @foreach ($kategori as $index => $k)
                        @if ($k->produk->count() > 0)
                            {{-- PENAMBAHAN CLASS kategori-baris-extra AGAR CSS BISA MEMAKSA TAMPIL --}}
                            <div class="mb-10 {{ $index >= 2 ? 'kategori-baris-extra' : '' }}" 
                                    ...
                                    <div class="flex overflow-x-auto gap-4 pb-6 px-4 sm:px-0 scrollbar-hide snap-x snap-mandatory">
                                        @foreach ($k->produk as $p)
                                            <div class="min-w-[170px] sm:min-w-[200px] md:min-w-[220px] snap-start">
                                                <div x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false" class="group relative bg-white rounded-2xl border border-black/5 hover:border-[#E68757] hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                                                    <a href="{{ route('produk.show', $p) }}" class="block p-3 flex-grow text-left">
                                                        <div class="relative rounded-xl overflow-hidden bg-white">
                                                            <img src="{{ asset('produk_assets/' . basename($p->gambar)) }}" class="w-full h-40 object-contain transition-transform duration-500 group-hover:scale-110" />
                                                        </div>
                    ...
                    @if ($kategori->count() > 2)
                        <div class="mt-4 mb-10 text-center" x-show="!showAll">
                            <button @click="showAll = true" class="inline-block px-10 py-3 bg-[#5B000B] text-white text-[10px] font-black rounded-2xl shadow-lg uppercase tracking-widest active:scale-95 transition-all">
